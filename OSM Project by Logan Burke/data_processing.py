import cleaning # 'cleaning' is a personal method created for cleaning the data
import csv
import codecs
import pprint
import re
	...
	if element.tag == 'node':
		...
		for t in element.iter('tag'):
			...
			if not(':' in t.attrib['k']):
				...
				t_dict['key'] = value
				t_dict['type'] = val[0]
			# fixing tags by passing them to 'cleaning'
			if t.attrib['k'] == 'addr:street':
				t_dict['value'] = cleaning.update_addr_name(t.attrib['v'])
			elif t.attrib['k'] == 'addr:city':
				t_dict['value'] = cleaning.update_city_name(t.attrib['v'])
			elif t.attrib['k'] == 'addr:postcode':
				t_dict['value'] = cleaning.update_postcode(t.attrib['v'])
			elif t.attrib['k'] == 'phone':
				t_dict['value'] = cleaning.update_phone(t.attrib['v'])
			else:
		...
		for t in element.iter('tag'):
			...
			if not(':' in t.attrib['k']):
				...
				t_dict['key'] = val[1]
				t_dict['type'] = val[0]
			#fixing ways by passing them to 'cleaning'
			if t.attrib['k'] == 'addr:street':
				t_dict['value'] = cleaning.update_addr_name(t.attrib['v'])
			elif t.attrib['k'] == 'addr:city':
				t_dict['value'] = cleaning.update_city_name(t.attrib['v'])
				...
				t_dict['value'] = cleaning.update_phone(t.attrib['v'])
			elif t.attrib['k'] == 'maxspeed':
				t_dict['value'] = cleaning.speedlimit(t.attrib['v'])
			else:
				t_dict['value'] = t.attrib['v']
			tags.append(t_dict)
		p = 0
